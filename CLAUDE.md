...
> 여기 없는 사실은 지어내지 말고 대리님께 묻는다. 마지막 갱신: 2026-09-08.
...
- **입금 처리**: 우리은행 거래내역 엑셀을 대리님이 보내면 대시보드에 없는 거래만 등록한다. 카드 결제는 카드사 정산(롯데·신한·현대·하나·NH…)으로 수수료가 빠진 금액이 들어오므로 학생과 따로 연결해야 하고, 수강료 필드에는 실제 입금액(예: 328,680원)을 기록한다. 4대보험 과오납 환급은 `general`, 대리입찰 컨설팅비는 `consulting`.
- **경매 업무**: 현황판에서 물건을 골라 권리분석을 한다. 권리분석 완료 체크 → 대시보드 데일리 경매분석에 자동 등록(조사중, 분석글 미작성). 대리입찰 컨설팅도 한다. 경매 물건 블로그 글은 `.claude/skills/naver-auction-blog-writer` 규칙을 따른다.
