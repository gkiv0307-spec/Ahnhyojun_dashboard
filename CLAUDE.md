...
- **우리 블로그는 셋이다.** 발행 확인(`-verify` 청크)에서 아래 셋은 모두 "발행완료"로 채택한다. 예전에는 공식블로그만 인정해서, 대표님 블로그에 올라간 글이 계속 발행대기로 남아 적체를 키웠다.
  - `https://blog.naver.com/ykphone_edu` — 공식블로그 (경매 물건 분석글 위주)
  - `https://blog.naver.com/hjko0` — 대표님 블로그 (학원 모집·정보성 글 위주)
  - `https://blog.naver.com/rhghwjd12`
  주의: 네이버 검색 API는 블로그 주인으로 거르지 못하고 검색어로만 찾는다. "옆커폰부동산에듀" 검색에 안 걸리는 글(브랜드명이 본문에 없는 글)은 제목의 핵심 키워드로 따로 검색해야 찾을 수 있다. 2026-09-11 기준 rhghwjd12 는 이 브랜드 검색에 한 건도 안 잡혔다.
- **대시보드 제목과 실제 게시 제목이 다르다.** 대리님이 올릴 때 제목을 다시 쓰는 경우가 많아 제목만으로는 자동 매칭이 안 된다. 그래서 게시 직후 "발행 완료" 버튼 + 글 주소 붙여넣기가 중요하다.
