...
(function () {
  ...
  // "add"(지금 올림) / "wait"(아직 2주 밖·기일 미정) / "drop"(기일이 지났음)
  ...
  function mergeMarketChunksIfNeeded(onDone) {
    ...
    resolveDriveFolder().then(function (folderId) {
      ...
      var added = 0, updated = 0, deferred = [], waiting = [], waitingNew = 0, dropped = 0;
      var today = todayStr();
      ordered.forEach(function (m) {
        ...
        if (existing) {
          ...
          if (marketAutoAddDecision(m, today) === "drop") { dropped++; return; }
      ...
      persistLocal();
      if (added || updated || waitingNew || dropped) {
        var parts = [];
...
"</div>";
        ...
        html += '<div class="stage-card-price">' + (m.minSalePrice ? won(m.minSalePrice) : '<span class="meta" style="font-weight:400">최저매각가 미입력</span>') +
          (discount !== null ? " " + pill(discount.toFixed(1) + "%", discount >= 50 ? "danger" : discount >= 30 ? "warn" : "neutral") : "") + "</div>";
