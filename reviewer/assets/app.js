...
(function (global) {
  ...
  function renderMonth() {
    ...
    legend.appendChild(h('div', { class: 'month-legend' }, [
      ...
      h('span', { class: 'swatch', title: '예약 가능일로 등록되지 않은 날짜에 잡힌 예약입니다. 설정에서 그 날짜를 등록하면 정원·잔여가 계산됩니다.' },
        [h('span', { class: 'box', style: 'background:var(--mute-soft);border-color:var(--mute-line)' }), '등록 밖 예약']),
  ...
  function openDayDetail(branch, dateStr) {
    ...
    body.appendChild(h('p', { class: 'hint', text:
      info.registered
        ...
        : '예약 가능일로 등록되지 않은 날짜입니다. 예약 ' + people.length + '건이 잡혀 있습니다. 설정에서 이 날짜를 등록하면 정원·잔여가 계산됩니다.' }));
