...
두 링크 모두 열면 비밀번호 화면이 먼저 뜬다. 기본값은 **0000** 이며, 정의 위치는
`stay/build-preview.mjs` 의 `bundle({ ..., passcode })` 와 `stay/assets/gate.js` 의 기본값이다.
바꾸려면 값을 고치고 다시 빌드·배포한다.
...
특히 관리용 링크는 전 지점 예약·비용·개인정보가 모두 보이므로 직원용과 **다른 번호**를 쓰고,
