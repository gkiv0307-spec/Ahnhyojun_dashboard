...
우측 상단 사용자 칩을 누르면 현재 계정과 문의처를 확인할 수 있다.
(실제 운영에서는 `Auth.current()` 만 사내 세션으로 교체)

**운영 계정은 `옆커폰 부동산팀` 하나로 통합했다.** 부동산팀은 접수·승인·예약 진행·정산을
팀으로 함께 처리하므로 담당자를 나누어 표시할 필요가 없고, 팀원 개인 연락처를 화면에
노출하지 않기 위함이다. 노출 연락처는 팀 대표번호(이동현 사원 010-8347-2699) 하나다.

| 계정 | 권한 | 담당 |
|---|---|---|
| 옆커폰 부동산팀 | 관리자(`canApprove`) | 예약 접수 · 승인 · 예약 진행 · 정산 전 과정 |

역할(5종)과 권한 매트릭스는 그대로 살아 있다. 개인별 처리자 구분이 필요해지면
관리자 > 사용자 관리에서 팀원 계정을 추가하면 역할별 메뉴·전이 제한이 즉시 적용된다.
다만 계정이 하나인 동안에는 **처리 기록의 처리자가 모두 `옆커폰 부동산팀`으로 남는다** —
누가 승인했는지까지 남겨야 한다면 팀원 계정을 따로 만들어야 한다.
