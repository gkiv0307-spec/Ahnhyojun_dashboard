...
| 이현철 | 소장 | 일반 신청자 | 각 지역 가맹점 계약 |
| 유현록 | 주임 | 일반 신청자 | 매장 계약 관리 · 상권 분석 |
| 이재환 | 주임 | 일반 신청자 | 시공 현장관리 · 감리 · 실측 |
| 여가현 | 사원 | 일반 신청자 | 인테리어 모델링 · 견적 |

**전용 `approver`(승인 담당자) 계정은 두지 않았다.** 관리자 권한이 승인을 포함하므로
고호정 실장·문소진 팀장이 승인한다. 승인 전담자를 따로 둘지는 확인이 필요하다.

지점 담당자 계정 2개(`대구점 담당자`, `부산점 담당자`)는 **자리표시자**이며,
실제 지점 목록과 담당자를 받으면 교체한다. 화면에서 "자리표시자"로 표시된다.

초기 데이터는 첫 접속 시 자동 생성된다(최근 12개월치 예약 46건 · 처리로그 약 475건 · 숙소 5곳 · 공지 8건).
