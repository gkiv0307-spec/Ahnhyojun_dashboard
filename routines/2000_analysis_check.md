...
5. **D. 근무 기록 (반드시 마지막에)** — title `ahj_patch_chunk_YYYY-MM-DD_routine_analysis.json` 으로 업로드:
...
끝나면 **새로 체크한 물건(사건번호·단지명·글 게시일)**, **발행완료로 바꾼 글(제목·주소)**, **아직 글이 없어 미작성으로 남은 물건 목록**, 올린 파일명을 한국어로 짧게 알려줘.